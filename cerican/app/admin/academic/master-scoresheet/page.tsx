...
export default async function MasterScoreSheetPage({
  searchParams,
}: {
  ...
  const selectedClassId = class_id ?? (classes?.[0]?.id ?? "")
  ...
  if (selectedClassId && selectedTermId) {
    ...
    if (subjectIds.length > 0 && studentIds.length > 0) {
      ...
      // Compute Total, Average, and Ranks
  ...
  const currentClassName = classes?.find((c: any) => c.id === selectedClassId)?.name ?? ""
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 print:space-y-2 print:m-0 print:p-0 print:max-w-none">
      ...
      <div className="mb-2 flex items-center justify-between flex-wrap gap-4 print:hidden">
        <div>
          <Link href="/admin/academic" className="text-sm text-primary hover:underline mb-2 inline-block">
            ← Back to Academic
          </Link>
          <h1 className="text-2xl font-bold text-text">Master Score Sheet</h1>
          <p className="text-text-muted mt-1">Overview of all student scores, totals, averages, and ranks by class.</p>
        </div>
        <PrintButton />
      </div>

      <form className="bg-white border border-border rounded-xl p-5 shadow-sm space-y-4 print:hidden">
        ...
        <div className="flex justify-end pt-2 border-t border-slate-200">
          <button
            type="submit"
            className="px-6 py-2.5 bg-slate-900 text-white font-bold rounded-md hover:bg-slate-800 transition-colors text-sm shadow-sm"
          >
            🔍 View Master Sheet
          </button>
        </div>
      ...
      {selectedClassId && selectedTermId ? (
        students.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-lg border border-border text-text-muted">
            <p className="text-lg font-medium">No active students in selected class</p>
          </div>
        ) : subjects.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-lg border border-border text-text-muted">
            <p className="text-lg font-medium">No subjects found for this class</p>
          </div>
        ) : (
                ...
                {students.map((stu: any, idx: number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-1.5 px-1 text-sm print:text-[10px] font-bold text-gray-800 print:text-black text-center font-mono bg-primary-light/5 print:bg-transparent">
                        #{stat.rank}
                      </td>
                    </tr>
                  )
        ...
        <div className="text-center py-16 text-text-muted print:hidden">
          <p className="text-lg font-medium">Select a class and term to view the master score sheet</p>
        </div>
      )}
    </div>
  )
